...
function h = cylinder2P(R,N,r1,r2,varargin)

%[X, Y, Z] = cylinder2P(R, N,r1,r2,'filled') %'filled' is optional
...
s1 = spherePlot(r2(1),r2(2),r2(3),2*R(1));
hold on;
rgb = [242 95 115];
rgb = rgb/norm(rgb);
s2 = surf(X, Y, Z,'FaceColor','green');
h = hggroup;
s1.Parent = h;
s2.Parent = h;
...
% rgb = [242 95 115];
% rgb = rgb/norm(rgb);
% surf(X, Y, Z,'FaceColor',rgb);
% axis equal;
function sphereSurf = spherePlot(xc,yc,zc,radius)
%SPHEREPLOT Summary of this function goes here
